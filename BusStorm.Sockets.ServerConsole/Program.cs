...
namespace BusStorm.Sockets.ServerConsole
{
  public class Program
  {
    public static void Main()
    {
      ...
      Console.ReadLine();
      var server = new BusServerSocket<BusMessage>(new SimpleBusMessageBusProtocolFactory(encKey));
      var disp = new CompositeDisposable();
      var sub2 = server.Connections.Subscribe(
        next =>
        {
          var sub = next.ReceivedMessages.Subscribe(
            msg =>
            {
        ...
        error => Console.WriteLine("SERVER Connection error {0}", error.Message),
        () => Console.WriteLine("SERVER CONNECTION COMPLETE"));
      disp.Add(sub2);
