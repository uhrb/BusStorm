...
namespace BusStorm.Sockets.ServerLoadTest
{
  public class Program
  {
    ...
    private static async void SendAndReceiveWorker(object state)
    {
      var sleep = (int)state;
      var clientId = Guid.NewGuid();
      var client = new BusClientSocket<BusMessage>(_hostString, Convert.ToInt32(_portString), new SimpleBusMessageBusProtocolFactory(EncKey));
      client.Connect();
      try
      {
        while (true)
        {
          ...
          var msg = await client.SendAndReceiveAsync(
            new BusMessage
          {
