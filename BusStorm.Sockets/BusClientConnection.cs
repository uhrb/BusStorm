...
namespace BusStorm.Sockets
{
  public class BusClientConnection<T> where T : BusStormMessageBase
  {
    ...
    internal BusClientConnection(Socket socket, long connectionNumber, IBusProtocolFactory<T> factory)
      : this(factory)
    {
    ...
    public long ConnectionNumber { get; private set; }

    public IObservable<T> ReceivedMessages
    {
      get { return _receivedMessages.ObserveOn(NewThreadScheduler.Default); }
    }

    public IObservable<byte[]> SendedBytes
    {
      get { return _sendedBytes.ObserveOn(NewThreadScheduler.Default); }
    }
    ...
    private void AsyncSocketOperationCompleted(object sender, SocketAsyncEventArgs e)
    {
