...
namespace BusStorm.SimpleMessage
{
  public class SimpleBusMessageBusProtocolFactory : IBusProtocolFactory<BusMessage>
  {
    ...
    public byte[] SerializeMessage(BusMessage message)
    {
      ...
      {
        encryptData = new byte[] { };
      }
      else if (message.IsManualPayload)
      {
        encryptData = (byte[])message.Payload;
      }
      else
      {
      ...
      encryptData = new BusCryptor().EncryptData(encryptData, _encryptionKey);

      message.OnWirePayloadSize = encryptData.Length;
      var bb = message.To.ToByteArray()
        ...
        .Concat(BitConverter.GetBytes(encryptData.Length))
        .Concat(encryptData)
        .ToArray();
